...
nel=10
for (i in 1:n){
	selected_edges <- sample(colnames(edges[,7:ncol(edges)-4]), size=nel, replace=F)
	...
	r <- cbind(A1,C1,E1)
	write.table(r,"multivariate_null_gordon.txt", sep="\t", quote=F, append=T, col.names=F, row.names=F)
